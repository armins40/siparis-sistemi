...
import { getConversionAnalytics } from '@/lib/affiliate-analytics';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getConversionAnalytics(affiliateId);
    return NextResponse.json({ success: true, ...data });
  } catch (e) {
    console.error('Affiliate analytics error:', e);
